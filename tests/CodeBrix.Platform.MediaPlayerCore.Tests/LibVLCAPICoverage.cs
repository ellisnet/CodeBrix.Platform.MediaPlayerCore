...
public class LibVLCAPICoverage
{
    ...
    public async Task CheckLibVLCCoverage()
    {
        ...
        {
            libvlc3Symbols = (await httpClient.GetStringAsync(LibVLCSym3URL, TestContext.Current.CancellationToken)).Split(new[] { '\r', '\n' }).Where(s => !string.IsNullOrEmpty(s)).ToArray();
            libvlc3deprecatedSym = (await httpClient.GetStringAsync(LibVLCDeprecatedSymUrl, TestContext.Current.CancellationToken)).Split(new[] { '\r', '\n' }).Where(s => !string.IsNullOrEmpty(s)).ToArray();
        }
